...
\noindent
MixColumns is the multiplication of each column by the polynomial $3x^{3}+x^{2}+x+2$ modulo $x^4+1$. As each product of an element of an ideal and an arbritrary element of the ring is an element of the ideal as well, this operation doesn't change the ideal generated by the columns of the \textsc{AES}-Matrix. This meas that the column rank of the matrix does not change by MixColumns. As the the column rank equals the row rank of a matrix, we can assume that the number of $0$-columns and $0$-rows in $A_{mca}(AES)$ equals the number of $0$-columns and $0$-rows in $A_{mca}(S$-Box). \\
